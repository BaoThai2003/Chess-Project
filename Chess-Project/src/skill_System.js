...
window.skillSystem = {
  ...
  skills: {
    ...
    azw_shield: {
      ...
      cost: 2,
      description:
        "Tạo một lá chắn cho cả đội giảm sát thương 0.25 trong 3 lượt và áp dụng hiệu ứng hình ảnh lá chắn màu vàng nhạt.",
      execute: function (player) {
        ...
        window.gameState.activeEffects.push(effect);

        // Mark visual target positions (all friendly pieces) for the duration
        effect.positions = [];
        for (let r = 0; r < 8; r++) {
          for (let c = 0; c < 8; c++) {
    ...
    azw_hard_work: {
      id: "azw_hard_work",
      name: "AZW - Lao động khổ sai",
      cost: 2,
      description:
        "Tất cả quân cờ đồng minh đều bị mất 0.25 sát thương ngay lập tức, nhưng gây thêm 0.25 sát thương trong 3 lượt.",
      execute: function (player) {
        if (!window.gameState) return false;
        ...
        for (let key in window.gameState.pieceHealth) {
          ...
          if (!piece) continue;
          if (friendly.includes(piece)) {
            if (window.gameState.applyDamage) window.gameState.applyDamage(row, col, 0.25, "skill", null);
            else
              window.gameState.pieceHealth[key].current = Math.max(0, window.gameState.pieceHealth[key].current - 0.25);
            damagedPositions.push(key);
          }
        }
    ...
    azw_sandstorm: {
      ...
      execute: function (player) {
        ...
        if (isNaN(r) || r < 0 || r > 6) {
          alert("Invalid row. Sandstorm cancelled.");
          return false;
        }
        ...
        window.gameState.activeEffects.push(effect);
        window.gameState.updateEffectsDisplay();
        alert(`Sandstorm active on rows ${r} and ${r + 1} for 3 turns.`);
        return true;
      },
    },
